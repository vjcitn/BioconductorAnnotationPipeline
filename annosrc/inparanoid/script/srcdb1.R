...
##This little R script just exists to read in the contents of the ../12-Jul-2007/IPDATA/ dir, and then for each file, it takes the name of that file, reads splits the name into two parts, and then makes a table based on the name of the file, then that file is loaded up into the table.

#In script I will call this by just going to the current dir with the IPDATA dir (eg above) and just running it.



## library(pdInfoBuilder)
## source("/mnt/cpb_anno/mcarlson/proj/mcarlson/sqliteGen/annosrc")
...
## fileNames = list.files(path="./IPDATA")
...
##     ##Make a table
##     message(cat(paste("Creating table: ",species[1],"_",species[2],sep="")))
...
##     clnVals <- prepareData(file=paste("./IPDATA/",file,sep=""))
