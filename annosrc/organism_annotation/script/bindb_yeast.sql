...
--THERE ARE NO easy paths to ensembl for yeast.
...
CREATE TABLE locus_tags (
 _id INTEGER REFERENCES genes(_id),
 locus_tag TEXT
);

INSERT INTO locus_tags
 SELECT DISTINCT g._id, gi.locus_tag
 FROM genes as g, genesrc.gene_info as gi
 WHERE g.gene_id = gi.gene_id;

DELETE FROM locus_tags WHERE locus_tag = "-";

CREATE INDEX c24 ON locus_tags(_id);



CREATE TABLE sgd_ids (
 _id INTEGER REFERENCES genes(_id),
 sgd_id TEXT
);

INSERT INTO sgd_ids
 SELECT DISTINCT g._id, ltrim(ltrim(xref.dbXref,"SGD"),":")
 FROM genes as g, genesrc.gene_dbXref as xref
 WHERE g.gene_id = xref.gene_id;

DELETE FROM sgd_ids WHERE sgd_id = "-";

CREATE INDEX c26 ON sgd_ids(_id);
...
INSERT INTO chrlengths
 SELECT ltrim(chr,'chr'), length FROM gp.chrlength_yeast;
...
INSERT INTO go_bp
 ...
 FROM genes as g, go.go_bp_yeast as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_mf
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_mf_yeast as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_cc
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_cc_yeast as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_bp_all
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_bpall_yeast as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_mf_all
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_mfall_yeast as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_cc_all
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_ccall_yeast as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
--The inserts int kegg and ec must be improved if possible
--The results that make it into these tables are REALLY small...
--basically, I need to retrieve the systematic names for these, and then join on THOSE,
--INSTEAD of just joining on the gene symbols (which are often NOT the systematic names)

INSERT INTO kegg
 SELECT DISTINCT gi._id as _id, substr(k.pathway_id, -5, 5)
 FROM locus_tags as gi, kegg.pathway2gene as k 
 WHERE gi.locus_tag=k.gene_or_orf_id
 ORDER BY _id;
...
INSERT INTO ec 
 SELECT DISTINCT gi._id as _id, e.ec_id
 FROM locus_tags as gi, kegg.sce_ec as e
 WHERE gi.locus_tag=e.gene_or_orf_id
 ORDER BY _id;
...
-- 	g.start,
-- 	g.end
--  FROM genes, gp.chrloc_yeast as g
...
--  FROM genes as g, ipi.yeast_pfam as i
...
--  FROM genes as g, ipi.yeast_prosite as i
--  WHERE g.gene_id=i.gene_id
--  ORDER BY _id;
