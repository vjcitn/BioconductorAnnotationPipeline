...
--  FROM genes as g CROSS JOIN ens.arabidopsis_gene_ensembl as e
...
-- Need a table to hold the TAIR cross references
CREATE TEMP TABLE tair_orgi (
 _id INTEGER REFERENCES genes(_id),
 tair_id TEXT
);

INSERT INTO tair_orgi
 SELECT g._id, i.dbXref
 FROM genes as g CROSS JOIN genesrc.gene_dbXref as i
 WHERE i.gene_id=g.gene_id AND i.dbXref != "-"
 AND i.dbXref LIKE 'TAIR:%';
...
INSERT INTO tair
 SELECT DISTINCT _id, ltrim(ltrim(tair_id,'TAIR'),':')
 FROM tair_orgi;

DROP TABLE tair_orgi;
VACUUM;
CREATE INDEX tairInd ON tair(_id);
...
--  SELECT ltrim(chr,'chr'), length FROM gp.chrlength_arabidopsis
--  WHERE chr NOT LIKE "%_random" 
--  AND chr NOT LIKE "%_hap%";
...
INSERT INTO go_bp
 ...
 FROM genes as g, go.go_bp_arabidopsis as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_mf
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_mf_arabidopsis as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_cc
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_cc_arabidopsis as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_bp_all
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_bpall_arabidopsis as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_mf_all
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_mfall_arabidopsis as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO go_cc_all
 SELECT g._id as _id, i.go_id, i.relationship_type
 FROM genes as g, go.go_ccall_arabidopsis as i
 WHERE i.gene_id=g.gene_id
 ORDER BY _id;
...
INSERT INTO kegg
 ...
 FROM genes as g, kegg.ath_NCBI_pathway2gene as k 
...
INSERT INTO ec 
 ...
 FROM genes as g, kegg.ath_ncbi_ec as e
...
-- INSERT INTO metadata
--  SELECT * FROM gp.metadata_arabidopsis;
...
-- 	g.start,
-- 	g.end
--  FROM genes, gp.chrloc_arabidopsis as g
...
--  FROM genes as g, ipi.arabidopsis_pfam as i
...
--  FROM genes as g, ipi.arabidopsis_prosite as i
--  WHERE g.gene_id=i.gene_id
--  ORDER BY _id;
