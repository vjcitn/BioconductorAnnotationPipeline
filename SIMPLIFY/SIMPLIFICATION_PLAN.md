...
6. **Yeast Ensembl IDs**: Resolved (2026-Aug). The active `ATTACH DATABASE 'ensembl.sqlite'` block in `bindb_yeast.sql` was replaced with the NCBI `gene_dbXref` fallback that was already present but commented out. Ensembl IDs for yeast are now sourced from NCBI gene cross-references (`Ensembl:` prefix entries), using `substr(ensid, 9)` to strip the prefix. The Ensembl provider dependency is fully removed for all species.

7. **Pfam provider: switch from `Pfam-A.full.gz` to `Pfam-A.regions.uniprot.tsv.gz`**: The current `providers/pfam/Makefile` downloads `Pfam-A.full.gz` — the complete Stockholm-format multiple sequence alignment file at ~24 GB compressed (~200 GB uncompressed). The pipeline then streams through it with `zcat | awk` to extract `#=GF` (family metadata) and `#=GS` (sequence-to-family mappings) records, taking several hours. The only information ultimately needed is: UniProt accession → Pfam family accession, plus family-level metadata (ID, description, type). EBI publishes `Pfam-A.regions.uniprot.tsv.gz` — a pre-extracted TSV of exactly this mapping — at roughly 400 MB compressed. Once the UniProt provider is in place (providing the gene ID → UniProt bridge), the Pfam provider can be rewritten to download this TSV instead, eliminating the 24 GB download and multi-hour parse step entirely. The current `Pfam-A.full.gz`-based build should be allowed to complete for this release cycle to confirm correctness; the switch to `Pfam-A.regions.uniprot.tsv.gz` is recommended for all subsequent rebuilds.
